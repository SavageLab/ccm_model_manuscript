...
We can completely analyze the behavior of the CCM system in terms of multiple parameters though analysis of the analytic solutions. However, to give intuition for which parameters are important near our current parameter regime we provide a sensitivity analysis of the cost of the CCM at cytosolic pH 7.3, the measured value in the light, and pH 8.3, the measured value in the dark. We find that for all the parameters tested the total cost of the CCM is always higher at pH 7.3 than pH 8.3. However the change from baseline was not necessarily the same at pH 7.3 vs 8.3. 

The sensitivity analysis also found that order of magnitude changes in the cell membrane permeability to ${\rm CO_2}$ and increasing the permeability of the carboxysome to ${\rm HCO_3^-}$ by a ratio of 10, 100, or 1000 compared to the permeability to ${\rm CO_2}$ have $<1\%$ change from baseline. Changing the target inorganic carbon pool (cytosolic ${\rm HCO_3^-}$) from 30 to 5, 15 and 25 mM  has between 5 and 20 \% change at pH 7.3 a $<2\%$ change from the pH = 8.3 baseline. 

Changing the membrane permeability to ${\rm HCO_3^-}$ has a much more dramatic effect on the cost, even if we re-optimize the permeability for each case. Increasing the permeability of the carboxysome to all inorganic carbon also has a very large effect. Finally we test the sensitivity of our analysis to a 2-fold decrease and increase in the cost of each ${\rm HCO_3^-}$ transport.

Next we use the analytic solutions for the CCM system when RuBisCO is saturated to further understand the parameter dependence. 
...
\section{Detailed analysis on the effect of cell membrane permeability on the CCM}
